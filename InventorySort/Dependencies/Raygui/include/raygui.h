...
RAYGUIAPI bool GuiValueBoxf(Rectangle bounds, const char *text, float *value, float minValue, float maxValue, bool editMode);    // Value Box control, updates input text with numbers
...
bool GuiValueBoxf(Rectangle bounds, const char* text, float* value, float minValue, float maxValue, bool editMode)
{
#if !defined(RAYGUI_VALUEBOX_MAX_CHARS)
#define RAYGUI_VALUEBOX_MAX_CHARS  32
#endif
	...
	static char textValue[RAYGUI_VALUEBOX_MAX_CHARS + 1] = "\0";
    static bool initValue = true;

	if (initValue) {
		// Calculate the integer part of the new value
		int newIntegerPart = static_cast<int>(*value);
		...
		// Calculate the integer part of the original value
		int originalIntegerPart = static_cast<int>(std::atof(textValue));

		int numDigits = count_digits(originalIntegerPart);

		char fmt[8];

		std::strcpy(fmt, "%");
		std::strcat(fmt, std::to_string(numDigits).c_str());
		std::strcat(fmt, ".2f");

		sprintf(textValue, fmt, *value);
		initValue = false;
	}
	...
	if ((state != STATE_DISABLED) && !guiLocked)
	{
		...
		if (editMode)
		{
			...
			int keyCount = (int)strlen(textValue);
			bool hasDecimalPoint = strchr(textValue, '.') != NULL;

			// Only allow keys in range [48..57] and the period (46)
			if (keyCount < RAYGUI_VALUEBOX_MAX_CHARS)
			{
				if (GetTextWidth(textValue) < bounds.width)
				{
					int key = GetCharPressed();
					if (((key >= 48) && (key <= 57)) || (!hasDecimalPoint && key == 46))
					{
						// Check if the integer part would exceed the maximum allowed length (5 characters)
						if (!(key >= 48 && key <= 57 && !hasDecimalPoint && keyCount >= 5))
						{
							textValue[keyCount] = (char)key;
							textValue[keyCount + 1] = '\0';
							keyCount++;
							valueHasChanged = true;
							hasDecimalPoint = strchr(textValue, '.') != NULL;
						}
					}
				}
			}
			...
			if (valueHasChanged)
			{
				if (strlen(textValue) > 0) // Check if the string is not empty
				{
					try
					{
						float newValue = std::stod(textValue);
			...
			if (IsKeyPressed(KEY_ENTER) || (!CheckCollisionPointRec(mousePoint, bounds) && IsMouseButtonPressed(MOUSE_LEFT_BUTTON))) {
				pressed = true;
				initValue = true; // Reset initValue flag to update the textValue when entering edit mode again
			}
		}
		else
		{
